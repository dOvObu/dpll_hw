...
struct lam : expr {
   ...
   vec<expr*> argsId; stmt* s;
   void enter(vis_base* s) { s->enter(this); }
};

struct func {
   func(string id, const vec<expr*>& argsId, body* body);
   string id; vec<expr*> argsId; body* _body;
   void enter(vis_base* s) { s->enter(this); }
};
...
struct STRUCT {
   ...
   string id; STRUCT* parent; vec<assign*> fields; vec<func*> methods;
   set<string> fieldIds, methodIds;
   void enter(vis_base* s) { s->enter(this); }
};
using struct_ptr = shared_ptr<STRUCT>;
extern vec<struct_ptr> structPool;

#endif // define PARSER_NODES_H
